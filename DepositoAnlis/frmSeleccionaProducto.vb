...
Public Class frmSeleccionaProducto
    ...
    Const nColCantidadPorUnidad = 11


    Const nColFiltroCodigo = 2
    Const nColFiltroLote = 3
    Const nColFiltroProducto = 4
    Const nColFiltroCantidad = 5
    Const nColFiltroUnidad = 6
    Const nColFiltroVto = 7
    Const nColFiltroDest = 8
    Const nColFiltroCantPorUnidad = 9
    Const nColFiltroUbicacion = 1
    ...
    Private Sub CargaListaDeProductos(Optional ByVal TodosLosRegistros As Boolean = False)
        ...
        ' eleva la cantidad de caracteres a buscar a 5 digitos 18/02/2021
        OptLote.Checked = ChkAuto.Checked And Len(TxtFiltro.Text) <= 5 And Val(TxtFiltro.Text) > 0
        OptCodigo.Checked = ChkAuto.Checked And Len(TxtFiltro.Text) > 5 And Val(TxtFiltro.Text) > 0
        ...
        strSQL = strSQL & " ,0  as Pedido , i.EntregaPorUnidad, s.ubica" 's.[Precio]" ' ,s.[Ubica]"
        ...
        Try
            ...
            For Each dr As DataRow In dt.Rows
                ...
                DgvFiltro.Item(nColFiltroCantPorUnidad, DgvFiltro.RowCount - 1).Value = dr.Item(8)
                DgvFiltro.Item(nColFiltroUbicacion, DgvFiltro.RowCount - 1).Value = dr.Item(9)
                DgvFiltro.Columns(3).Width = 50
                DgvFiltro.Columns(1).Width = 50
                DgvFiltro.Columns(6).Width = 50
                DgvFiltro.Columns(6).Width = 80
                DgvFiltro.Columns(7).Width = 70
                DgvFiltro.Columns(8).Width = 50
                DgvFiltro.Columns(9).Width = 50
        ...
        For nFiltro = 0 To DgvFiltro.RowCount - 1
            If DgvFiltro.Rows(nFiltro).Cells(0).Value Then
                ...
                If Not LoteExistente Then
                    ...
                    FrmRecepStock.DGVR.Item(nColConsUbica, nfila).Value = DgvFiltro.Item(nColFiltroUbicacion, nFiltro).Value
                    FrmRecepStock.DGVR.Item(nColConsDestinatario, nfila).Value = DgvFiltro.Item(nColFiltroDest, nFiltro).Value
                    FrmRecepStock.DGVR.Item(nColCantidadPorUnidad, nfila).Value = DgvFiltro.Item(nColFiltroCantPorUnidad, nFiltro).Value

                    FrmRecepStock.DGVR.Columns(3).Width = 50
                    FrmRecepStock.DGVR.Columns(1).Width = 50
                    FrmRecepStock.DGVR.Columns(6).Width = 50
                    FrmRecepStock.DGVR.Columns(10).Width = 50
                    FrmRecepStock.DGVR.Columns(7).Width = 70
                    FrmRecepStock.DGVR.Columns(8).Width = 50
                    FrmRecepStock.DGVR.Columns(9).Width = 50
